...
export function Navbar({ topOffset = 0 }: { topOffset?: number }) {
  ...
  // Load the logged-in Discord user (if any).
  useEffect(() => {
        ...
        <div className="flex items-center gap-2.5">
          {user ? (
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-56 overflow-hidden rounded-2xl border border-outline-variant bg-surface-container-lowest shadow-[0_12px_40px_rgba(23,28,31,0.18)]">
                  <div className="flex items-center gap-3 border-b border-outline-variant px-4 py-3">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-[14px] font-semibold text-on-surface">{user.username}</div>
                      <div className="text-[12px] text-on-surface-variant">Connected via Discord</div>
                    </div>
                  </div>
                  <a href="https://discord.gg/7AYW4693XQ" target="_blank" rel="noopener noreferrer" className="block px-4 py-2.5 text-[14px] text-on-surface transition-colors hover:bg-surface-container">
                    Open Discord server
                  </a>
                  <a href="/api/discord/logout" className="block px-4 py-2.5 text-[14px] text-error transition-colors hover:bg-error/10">
                    Log out
                  </a>
                </div>
              )}
